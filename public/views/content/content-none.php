...
<article id="post-0" <?php post_class( 'post' ); ?>>
	<header class="entry-header">
		<h1 class="entry-title"><?php esc_html_e( 'Nothing Found', 'silver-quantum' ); ?></h1>
	</header>
		...
		<?php if ( is_home() && current_user_can( 'publish_posts' ) ) : ?>
			<p>
				<?php
					...
					printf( esc_html__( 'Ready to publish your first post? %s', 'silver-quantum' ), '<a href="' . esc_url( admin_url( 'post-new.php' ) ) . '">' . esc_html__( 'Get Started', 'silver-quantum' ) . '</a>' );
				?>
			</p>
		<?php else : ?>
