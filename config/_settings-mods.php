...
return [
	...
	'theme_footer_custom_credit' => function() {
		...
		$copyright = sprintf( __( "&#169; %1\$s. %2\$s.", 'silver-quantum' ), $year, Backdrop\Theme\Site\render_home_link() );

		if ( is_classicpress() ) {
			$footer_text = sprintf( __( "Powered by %1\$s and %2\$s.", 'silver-quantum' ), Backdrop\Theme\Site\render_cp_link(), Backdrop\Theme\Site\render_theme_link() );
		} else {
			$footer_text = sprintf( __( "Powered by %1\$s and %2\$s.", 'silver-quantum' ), Backdrop\Theme\Site\render_wp_link(), Backdrop\Theme\Site\render_theme_link() );
		}
